...
def cut_by_rectangle(src1, style, start_x, start_y, end_x, end_y):
    ...
    image_part = img1[start_x:end_x, start_y:end_y]  # 截取第5行到89行的第500列到630列的区域
    fgqi_result = fg_run(style, image_part)
    img2 = cv.imread(fgqi_result)  # logo图像，要往原始图像上添加
    # img2 = cv.resize(img2, ((end_y - start_y), (end_x - start_x)))
    roi = img1[start_x:end_x, start_y:end_y]  # 在原始图像中截取logo图像大小的部分

    img2gray = cv.cvtColor(img2, cv.COLOR_BGR2GRAY)  # 将logo图像灰度化
    ret, mask = cv.threshold(img2gray, 200, 255, cv.THRESH_BINARY)  # 将logo灰度图二值化，将得到的图像赋值给mask，logo部分的值为255，白色
    mask_inv = cv.bitwise_not(mask)  # 将mask按位取反，即白变黑，黑变白

    img1_bg = cv.bitwise_and(roi, roi, mask=mask)  # 将原始图像中截取的部分做处理，mask中黑色部分按位与运算，即保留黑色部分，保留除logo位置外的部分
    img2_fg = cv.bitwise_and(img2, img2, mask=mask_inv)  # 将logo图像中，mask_inv部分按位与运算，即保留黑色部分，保留logo
...
def cut_with_pixelArrays(src, style, pixelArrays):
    ...
    mask_inv = cv.bitwise_not(mask)  # 将mask按位取反，即白变黑，黑变白
    img1_bg = cv.bitwise_and(pixelArrays, pixelArrays, mask=mask)  # 将原始图像中截取的部分做处理，mask中黑色部分按位与运算，即保留黑色部分，保留除logo位置外的部分
    img2_fg = cv.bitwise_and(new_part_img, new_part_img, mask=mask_inv)  # 将logo图像中，mask_inv部分按位与运算，即保留黑色部分，保留logo
