var penCutout = function () {
  ...
  this.fillBackColor = function () {
    //  这里是用每个 96* 96 的背景小方块填充画布
    // for (var i = 0; i < this.img.w; i += 96) {
    //   for (var j = 0; j <= this.img.h; j += 96) {
    //     this.can.canvas.drawImage(this.can.imgBack, i, j, 96, 96);
    //   }
    // }
    this.can.canvas.drawImage(this.can.imgBack, 0, 0, this.img.w, this.img.h)
    // 在源图像外显示目标图像，即在用户圈出的闭合区间外填充背景颜色
  ...
  // 返回矩形的左上角和右下角
  this.xzbReturnMatrix = function () {
    var tempPointArray; // 用户抠图区域的左上角和右下角的两个点
    var tempPointList; // 坐标点集合
  ...
  // 将迁移后矩形图像填充回原图像
  // resImgPath：迁移后的图像保存路径
  this.xzbDrawToSrc = function (resImgPath) {
    var tempPointArray; // 用户抠图区域的左上角和右下角的两个点
    var tempPointList; // 坐标点集合
    if (this.notEmptyObj(this.can.pointList) && this.can.pointList.length > 1) {
      tempPointList = JSON.parse(JSON.stringify(this.can.pointList));
    ...
    img.onload = function () {
      ...
      ctx.drawImage(img, tempPointArray[0].pointx, tempPointArray[0].pointy, img.width, img.height);
    };
    this.fillBackColor();
  }
  ...
  // 根据用户移动的坐标数组，算出左上角和右下角的两个点，以此来构造矩形
